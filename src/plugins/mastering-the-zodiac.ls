...
mastering-the-zodiac = ({config, G, set_config, set_data}) ->
  onload {G} # this is just a function that has the stuff needed for the loading
    ...
    s \svg, width: G.width, height: G.height, ->
      return
        * s \g.houses, !->
            ...
            for house in houses
            # each houses, (house) !->
              mid_deg := house.a + 15 - 120
              if mid_deg < 0
                mid_deg += 360
              middle = polar-to-cartesian CENTER_X, CENTER_Y, HOUSES_MID, mid_deg
              els.push \
                s \path.house,
                  fill: house.c
                  stroke: house.stroke
                  d: describe-cone-section CENTER_X, CENTER_Y, HOUSES_INNER, HOUSES_OUTER, house.a - 120, house.a + 30 - 120
              els.push \
                s \text.house-text,
                  fill: house.c
                  stroke: house.stroke
        ...
        * s \g.wheel,
            ...
            s \g.signs, ->
              ...
              for sign, i in signs
                a = 360 - sign.a
                b = 360 - signs[if i is 12 => 0 else i+1].a
                mpa = 5 + a - b
                if mpa < 0
                  mpa += 360
                mid_deg = a - 90 - (mpa / 2)
                if mid_deg > 360
                  mid_deg -= 360
                middle = polar-to-cartesian CENTER_X, CENTER_Y, 222, mid_deg
                ss = symbols[i]
